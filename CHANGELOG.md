...
All notable changes to `coolms/entity-bundle` are recorded here.
...
```
composer require coolms/entity-bundle:^2.0 coolms/core-doctrine:^2.0 coolms/entity-doctrine:^2.0
```
...
A bare `composer require coolms/entity-bundle` does not install the wrong
...
- `coolms/entity-module`: `^1.0` to `^2.0`
...
First release. Symfony integration for `coolms/entity`: the bundle and its
extension, the extras infrastructure compiler passes, API Platform property
metadata factories for dynamic fields, and the entity search endpoint.
